...
export default function DevPage() {
  ...
  const createInvite = async (event) => {
    ...
    if (!adminToken) {
      setInviteError("관리자 토큰을 먼저 입력해주세요.");
      return;
    }
      ...
      <div className="panel stack gap-lg">
        <div className="panel-heading">
          <div>
            ...
            <p className="muted">
              초대 코드 발행과 만료 관리용 페이지입니다. 관리자 토큰이 필요하며,
              토큰은 브라우저에만 저장됩니다.
            </p>
          </div>
        </div>
        ...
        <div className="stack gap-md">
          <label className="field">
            ...
            <input
              ...
              placeholder="환경변수 ADMIN_TOKEN과 동일한 값"
            />
          </label>
          <p className="status">
            토큰은 로컬 스토리지에 저장되어 api.zcxv.app에서만 사용됩니다. 새 토큰을
            발급했다면 다시 입력해주세요.
          </p>
        </div>
      </div>
